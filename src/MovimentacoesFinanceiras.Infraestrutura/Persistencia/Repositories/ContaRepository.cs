...
internal sealed class ContaRepository(BancoDadosContext contexto) : IContaRepository
{
        ...
        {
                var query = _contexto.Lancamentos.Where(l => l.ContaId == contaId && l.CriadoEm <= dataReferencia);
                var creditos = await query.Where(l => l.Tipo == TipoLancamento.Credito).SumAsync(l => l.Valor, cancellationToken);
                var debitos = await query.Where(l => l.Tipo == TipoLancamento.Debito).SumAsync(l => l.Valor, cancellationToken);
